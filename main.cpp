...
int main(int argc, char const *argv[])
{
    try
    {
        ...
        new_pool = sort_by_condition(ip_pool, condition_func1);
        show_ip_pool(new_pool);
        std::cout << "---------------------------------" << std::endl;
        auto condition_func2 = [](std::vector<std::vector <std::string>>::iterator it)-> bool {return (*it)[0]=="46" && (*it)[1]=="70";};
        new_pool = sort_by_condition(ip_pool, condition_func2);
        show_ip_pool(new_pool);
        std::cout << "---------------------------------" << std::endl;
        ...
        new_pool = sort_by_condition(ip_pool, condition_func3);
        show_ip_pool(new_pool);
    }
